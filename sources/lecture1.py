...
def insertion_search(a):
    ...
    # решается при помощи смены указателей»
    # — Википедия
...
def merge_sort(a):
    ...
    # __Время работы__ `O(nlogn)`.

    # Если время работы алгоритма на массиве длинны n — `T[n]`,
    # то по определению справедливо рекуррентное соотношение
    # `T[n] = 2T[n/2] + n`: две сортировки массивов размера
    # n/2 и еще n операций на слияние.

    # Можно убедиться, что `T[n] = n/2 + nlogn / log2 = O(nlogn)`.

assert insertion_search(deepcopy(a)) == merge_sort(deepcopy(a)) == list(range(100))
