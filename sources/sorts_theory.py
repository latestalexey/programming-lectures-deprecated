...
Материалы: лекция 1 (13.01), 3 (20.01).
...
входных данных размера \\(n \\) нам нужно знать, сколько операций совершит алгоритм
...
справедливо соотношение \\(T[n] = a T[n/b] + O(n^d) \\).

Теорема утверждает, что

\\(T[n] =
\begin{cases}
	O(n^d\log n),		& \mbox{если } a = b^d \\\\
	O(n^d),				& \mbox{если } a < b^d \\\\
	O(n^{\log_b a}),	& \mbox{если } a > b^d
\end{cases}\\)
